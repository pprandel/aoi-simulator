...
def calc_analitic_aoi(ro_0, ro_1, mu):
    def calc_age(ro_i, ro_menos_i, mu):
        ro = ro_i + ro_menos_i
        return (1/mu) * ( ((1+ro+ro**2)**2+2*ro**3) / ((1+ro+ro**2)*((1+ro)**2)) + (1 + (ro**2/(1+ro)))*(1/ro_i) ) 
    aoi = {}
...
for ro in RO:
    ...
    for percent in np.arange(0, 1.1, 0.1):
        ...
        arq_nome = "experimentos/duas_fontes/mm1_lcfs_w/ro_" + str(ro) + "_" + str(ro_0) + "_" + str(ro_1) + ".txt"
        sim_aoi[ro_str].append(mean_aoi_n_fontes_lcfs(arq_nome, 2))
        analit_aoi[ro_str].append(calc_analitic_aoi(ro_0, ro_1, 1))

with open("resultados/sim_duas_fontes_mm1_lcfs_w_aoi.txt", 'w') as f:
    json.dump(sim_aoi, f, indent=2)

with open("resultados/analit_duas_fontes_mm1_lcfs_w_aoi.txt", 'w') as f:
